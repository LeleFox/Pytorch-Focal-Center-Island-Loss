...
$
\mathcal{L}_{C} = \frac{1}{2} \sum_{i=1}^{N} \left\lVert x_i - c_{y_{i}} \right\rVert_{2}
$
...
$
c_{j}^{t+1} = c_{j}^{t} - \alpha \, \mathrm{d}c_{j}^{t}
$

Where $\mathrm{d}c_{j}^{t}$ is the gradient of the center loss with respect to the class center $c_{j}$.
